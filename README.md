...
首次从 GitHub 下载 `.exe` 并运行时，Windows Defender SmartScreen 可能显示警告「未识别的应用程序」。这是因为本应用为开源项目，暂未使用付费数字签名证书，属于正常现象。

按以下步骤即可安全运行：

1. 双击 `.exe` 后，点击左下角「**更多信息**」
2. 点击「**仍要运行**」
3. 应用将正常启动，此后再次运行不会再提示

> 如果你从可信来源（GitHub 发布页面、本仓库）下载文件，可以放心使用。SmartScreen 警告会随着下载量增加自动降级。
