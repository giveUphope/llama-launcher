...
- 界面基于 **Arco Design Vue** 设计系统（2026-09 全站迁移）：统一设计 Token（CSS Variables）、4px 扁平化圆角、组件级实底/阴影，状态色走 Arco 语义色板（`--primary-*` / `--success-*` 等），深浅主题全站兼容

- 布局延续顶栏模型选择、可折叠侧边栏、底部状态栏的桌面范式

- 支持 **浅色 / 深色主题** 切换（跟随系统可配，在「应用设置 → 外观与语言」调整）
...
- 主题与语言统一由本页「外观与语言」卡片调整（浅色 / 深色 / 跟随系统即时切换）
