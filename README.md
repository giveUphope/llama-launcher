...
- **60 个** `llama-server` 参数，按 13 个分区归类（网络 / 上下文 / KV 缓存 / 采样 / 推测解码…），基线对齐 llama.cpp **b11053**。
...
- **E2E**：`pnpm e2e:web`（渲染层）/ `pnpm e2e:electron`（Electron 冒烟），首次需 `pnpm exec playwright install chromium`，详见 [testing.md](docs/testing.md)。
...
依赖单向：`desktop → core + shared`、`core → shared`、`ui → shared`，`shared` 不依赖任何上层包。完整目录树与依赖流见 [architecture.md](docs/architecture.md) §2–3；开发者约定与构建陷阱见 [AGENTS.md](AGENTS.md)。
...
全部文档在 `docs/`，本文件是仓库**唯一 README**。想弄明白某件事对应读哪一篇：

| 想弄明白…                               | 读这里                                                                 |
| ----------------------------------- | ------------------------------------------------------------------- |
| 60 个参数与 `--help` 的逐条对照              | [params/LLAMA\_SERVER\_PARAMS.md](docs/params/LLAMA_SERVER_PARAMS.md) |
| 整体结构、目录树、Monorepo 依赖流               | [architecture.md](docs/architecture.md)                             |
| 核心模块（进程、命令构建、GGUF、下载、清理）与关键函数索引     | [core-modules.md](docs/core-modules.md)                             |
| 参数系统与双轨（会话 / 预设）、依赖联动、控件组件          | [params-system.md](docs/params-system.md)                           |
| Electron 主进程（窗口、IPC 注册、托盘、preload）  | [desktop-main.md](docs/desktop-main.md)                             |
| 56 个 IPC 通道清单（改 IPC 前必读）            | [ipc-channels.md](docs/ipc-channels.md)                             |
| 前端架构 + **UI 风格规范 §7.5**            | [frontend.md](docs/frontend.md)                                     |
| 类型定义与持久化（`settings.json` 全字段、预设格式） | [data-persistence.md](docs/data-persistence.md)                     |
| 打包配置与常见故障（junction 陷阱、输出目录锁定）      | [packaging.md](docs/packaging.md)                                   |
| CI/CD 与自动发版流水线                      | [ci-cd.md](docs/ci-cd.md) · [auto-release.md](docs/auto-release.md) |
| 测试结构与 E2E                           | [testing.md](docs/testing.md)                                       |
| 日常命令、提交与发版约定、**文档编写约定**            | [workflow.md](docs/workflow.md)                                     |
| 关键设计决策                              | [design-decisions.md](docs/design-decisions.md)                     |
| UI 风格待修复清单                          | [style/STYLE\_TODO.md](docs/style/STYLE_TODO.md)                    |
| 版本历史                                | [CHANGELOG.md](docs/CHANGELOG.md)                                   |
| 已结束的规划 / 实验 / 重构交接（只读归档）           | [archive/INDEX.md](docs/archive/INDEX.md)                           |

初次接触项目建议按 [architecture.md](docs/architecture.md) → [core-modules.md](docs/core-modules.md) → [frontend.md](docs/frontend.md) 的顺序读。
