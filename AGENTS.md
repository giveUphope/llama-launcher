...
- **英文不是自动产物**：`docs/en/**` 是与中文一一对应的**独立文件**，改中文**不会**自动改写英文（唯一的自动生成物是参数对照表——`generate-params-doc.cjs` 一次产出中英两份）。所以「只改中文就收工」在本仓库等于任务未完成，英文镜像必须同轮手工同步。门禁能替你兜住的是**数字与链接**这一层：两树的通道数 / 参数总数 / 分组数 / 版本号不相等会直接 fail（`verify-ipc-sync` / `verify-params-sync` / `verify-version-sync` 都递归覆盖两树），英文侧漏写或改错声明同样 fail；但**散文语义的漂移没有机器检查**，只能靠下面的结构对账纪律。
- **同步纪律（改完中文后对英文做三件事）**：① 标题数与标题编号逐节对齐（两树 `##` 数必须相等）；② 表格行数、围栏数对齐；③ 数字 token 集合对齐（`grep -o '[0-9]\+' | sort | uniq -c` 两树对拍）。这三步是纯机械检查，几分钟就能做完，也是英文镜像不烂掉的唯一保障。
