...
| Unit tests                     | `pnpm test` (Vitest 4; `packages/core` 25 个测试文件 / 373 用例 + `packages/ui` 7 个测试文件 / 75 用例，turbo 一并运行)                                                          |
...
- **Command building** lives in `packages/core/src/command-builder.ts` (`buildCommand`). Emission basis is `engineDefault` (see the bullet above), never `ParamDef.default`; `float_slider` keeps 2 decimals; `checkbox` emits `flag`/`invert_flag` (no `invert_flag` + default-false emits only when true); `draft-model` is normalized to `draft-simple`.
