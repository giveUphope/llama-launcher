...
- `packages/shared/` — **single source of truth** for types, the 64-param table (`src/params/definitions.ts`), the param→argv emission rule (`src/params/command.ts`, `buildArgv`) and i18n (zh/en). All other packages depend on it.
...
| Unit tests                     | `pnpm test` (Vitest 4; `packages/core` 27 个测试文件 / 401 用例 + `packages/ui` 8 个测试文件 / 79 用例，turbo 一并运行)                                                          |
...
- **Dual-track param logic (no** **`_enabled`).** `PresetValues` is a plain value map; the old `_enabled` JSON string was removed — `buildCommand` emits a flag when the value differs from the **engine default** (`engineDefault` in `packages/shared/src/params/engine-baseline.ts` — **not** `ParamDef.default`, which is the UI initial value and carries the launcher's baseline recommendations like `-ctk q8_0` / `--load-mode none` / `--fit off`; using `default` as the basis kept those recommendations out of the command line forever), when the value is not one of that param's `sentinel` ("unspecified") values (checkbox: `flag` when true, `invert_flag` when false; a no-`invert_flag` default-false checkbox emits only when true), and `model` is always passed (`-m`). `verify-params-sync.cjs` hard-fails if `engineDefault` drifts from the `(default: X)` in `docs/params/llama-server-help-out.txt`, if the baseline table's key set ≠ `PARAMS`, or if a `default ≠ engineDefault` override lacks a `note`. The two tracks are: **临时轨道 (session)** — every edit auto-persists (800ms throttle) to `settings.session_values` + `session_baseline` in `~/.llama_launcher/settings.json`, restored on launch; **预设轨道 (presets)** — `<models_dir>/presets/*.json`, written only on explicit save. `hasChanges` is computed against the session baseline (`SessionBaseline { preset_name, values }`); switching models or applying a GGUF-suggested preset with unsaved changes triggers `confirmDiscardDirty()`; restore/clear entry points live on the Params page status bar (the baseline badge was removed as redundant in 2026-09).

- **Command building is split by concern.** The emission rule itself — a pure function of the param table — lives in `packages/shared/src/params/command.ts` (`buildArgv` / `argvFromPreviewOptions`), because the command has two consumers: the executor (core spawns the process) and the presenters (service-page preview, browser mock — no node filesystem). `packages/core/src/command-builder.ts` keeps only `buildCommand` (adds the `exePath` existence check) and `previewCommand`. Emission basis is `engineDefault` (see the bullet above), never `ParamDef.default`; `float_slider` keeps 2 decimals; `checkbox` emits `flag`/`invert_flag` (no `invert_flag` + default-false emits only when true); `draft-model` is normalized to `draft-simple`. **Never fork a second copy for display purposes** — `verify-params-sync.cjs` ⑤ fails `pnpm lint` if any file other than `shared/params/command.ts` assembles `p.flag` (the mock's own simplified builder caused a preview that silently dropped the baseline recommendations, removed 2026-09-26). **Skip-emitting rests on a premise the UI must state**: "value == engine default ⇒ don't emit" assumes nothing else rewrites that default, but llama.cpp also reads `LLAMA_ARG_*` env vars (b11178 baseline: 57 of 60 app params, and a malformed env value aborts the process even when the CLI passes a valid one). `Launcher` therefore reports `ServerInfo.envOverrides` and the preview card says those overrides never appear in the command. `--host` accepts comma-separated multiple addresses since b11178: parse it only via shared `hostList`/`tcpHosts`/`displayHost` (raw string as one address → unusable URL and a port probe that reports "occupied" as "free").
- **The only *verified* parameter signal is the `/props` read-back.** After readiness, `Launcher` GETs `/props` and reconciles it against what it sent (`shared/params/props-mapping.ts` → `checkEngineProps`, 14 mapped params — the other ~50 are never reported by the engine, so the UI must stay silent unless there is a real mismatch and must never imply "all checked"). It re-checks **on demand, never on a timer** — `Launcher.recheckProps()` runs when a tab becomes visible again (`server:status(refresh:true)`) or the user clicks re-verify, and an event is re-emitted only when the serialized result changed (`PropsCheck.checkedAt` carries freshness), and the same channel reports `baselineDrift` when `/props` `build_info` no longer matches `ENGINE_BASELINE_BUILD` — the baseline table is a snapshot of one engine version, and this is the only way to catch that at runtime. Two rules to keep: (1) comparisons need normalization — float32 noise (`0.95` reads back `0.949999988079071`), seed's uint32 wrap (`-1` reads back `4294967295`), Windows path separators/case; entries the engine derives itself (`model`/`alias`/`parallel`) are `onlyWhenSent`; (2) **a failed read-back is `error='unreachable'`, never a mismatch** — a transient network failure must not let the UI claim a parameter did not take effect. The result rides on the **existing** `server:status` channel via a second, same-status event (the read-back is async; do not add an IPC channel for it, and the renderer handles status idempotently). **Unit tests must inject `propsFetcher`** — the default uses global fetch and would really hit the user's own llama-server on :8080.
...
- **控制台输出是批量通道**：`IPC.SERVER_OUTPUT_BATCH`（`server:output-batch`，载荷 `OutputEntry[]`，16ms 窗口合并）取代旧的逐行 `server:output`。渲染层入口是 `server` store 的 `pushOutputBatch()`，preload 侧 API 名为 `server.onOutputBatch(cb)`。**新增/改通道一律走 `shared/src/types/ipc.ts` + `pnpm generate:ipc`**（当前 57 个通道，改名不增数）。
...
- **数字与标识符**：参数总数（64）、分组数、IPC 通道数（57）、像素/毫秒实测值在两树中必须完全一致，且都不翻译代码标识符、路径、命令、i18n 键；`verify-ipc-sync` / `verify-params-sync` 已递归覆盖两树，英文句式（"57 channels"）同样受检。
