...
以下每篇都有同名英文镜像（`docs/zh/x.md` ↔ `docs/en/x.md`），按语言取用即可；**内容以中文为准，英文是镜像**（规则见上一节「文档双语规则」）。
...
- `README.md` — 仓库根**英文**着陆页；`README.zh-CN.md` 是它的中文镜像（同结构、同数字），两个文件顶部各有 `> Language: English · [简体中文](README.zh-CN.md)` / `> 语言：中文 · [English](README.md)` 互链。着陆页只保留「是什么 / 亮点 / 快速开始 / 使用流程 / 项目结构 / 技术栈 / 文档地图 / 许可证」八块，深度细节一律在 `docs/`。徽章锚点依赖 `#tech-stack` / `#license`（中文版为 `#技术栈` / `#许可证`），改标题必跑 `pnpm docs:check`。**着陆页与文档地图的链接指向本语言树**：英文 README → `docs/en/*`，中文 README → `docs/zh/*`。
