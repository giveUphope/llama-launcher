...
- `packages/shared/` — **single source of truth** for types, the 60-param table (`src/params/definitions.ts`), the param→argv emission rule (`src/params/command.ts`, `buildArgv`) and i18n (zh/en). All other packages depend on it.
...
| Unit tests                     | `pnpm test` (Vitest 4; `packages/core` 27 个测试文件 / 394 用例 + `packages/ui` 8 个测试文件 / 79 用例，turbo 一并运行)                                                          |
