...
| Package Windows installer | `pnpm dist` (root; delegates to `@llama-launcher/desktop dist`, outputs `release/llama Launcher 0.0.02.exe`, Portable; uses `scripts/dist-with-fallback.cjs` for locked-output fallback) |
