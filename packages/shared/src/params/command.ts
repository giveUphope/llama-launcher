import { PARAMS } from './definitions.js';
import { engineDefaultOf, isSentinelValue, isUnsetValue, sameParamValue } from './engine-baseline.js';
import type { AppSettings, ParamDef, ParamDependsOn, PresetValues } from '../types/index.js';

/**
 * 参数表 → argv 的**唯一**发射实现。
 *
 * 为什么放 shared 而不是 core：发射规则是参数表的纯函数，而命令有两个消费方——
 * **执行方**（core spawn llama-server）与**展示方**（服务页预览框、浏览器 mock）。
 * 只要规则在 core 里，展示方就拿不到它，只能另抄一份简化版；改了发射规则那份副本
 * 会静默失真（2026-09-26 即为此修掉 mock 预览的并行实现）。放这里之后两边调同一个函数，
 * 结构上不存在漂移。需要 node 能力的守卫（exe 是否存在）留在 core 的 buildCommand 里。
 */
...
/**
 * 预览入参 → argv 入参的映射（exe 路径、`-m`、扩展参数开关）。
 * 展示方与执行方共用：core 拿它再叠一层 exe 存在性校验，浏览器 mock 直接喂 buildArgv。
 */
export function argvFromPreviewOptions(opts: PreviewOptions): ArgvOptions {
  return {
...
export function buildArgv(opts: ArgvOptions): string[] {
...
function shouldSkip(p: ParamDef, v: string | number | boolean, values: PresetValues): boolean {
 ...
 */
function isDependencyMet(dep: ParamDependsOn, values: PresetValues): boolean {
