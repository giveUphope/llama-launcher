export const zh = {
  ...
  col_model: '模型',
  col_actions: '操作',
  preset_dblclick_hint: '双击行可直接应用；保存同名预设即覆盖',
  preset_empty: '暂无预设——调整参数后在上方保存，将自动出现在此列表',
  ...
  msg_preset_delete_title: '删除预设',
  msg_preset_delete: '确定删除预设 "{0}"？此操作不可恢复。',
  msg_vram_unavailable: '显存估算不可用（未检测到计算设备或模型元数据未读取）',
  lbl_vram_occupancy: '显存占用(估算)',
  msg_occ_vram_line: '显存（{0}）：权重 {1} + KV {2} + 计算 {3} ≈ {4} GiB / 空闲 {5} GiB — {6}',
  occ_ok: '✓ 可行',
  occ_over: '⚠ 超出空闲显存',
  ...
  target_memory: '省显存',
  lbl_perf_target: '目标',
  target_picker_title: '性能目标：联动推荐关键杠杆（卸载层数 / 上下文 / KV 档位 / FA / 推测解码）',
  target_apply: '应用到参数',
  ...
  fit_partial: '部分卸载',
  fit_no: '建议降档',
  msg_fit_full_tip: '显存估算：全卸载可支撑上下文 ~{0}（KV {1}）',
  ...
  lbl_model_dir_missing: '模型目录不存在',
  lbl_model_dir_unset: '模型目录未设置',
