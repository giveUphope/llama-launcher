...
export const PARAM_LABELS: Record<string, ParamI18nEntry> = {
  ...
  cors_origins: { zh: '允许的来源', en: 'Allowed Origins' },
  cors_methods: { zh: '允许的方法', en: 'Allowed Methods' },
  cors_headers: { zh: '允许的请求头', en: 'Allowed Headers' },
  cors_credentials: { zh: '允许携带凭据', en: 'Allow Credentials' },
...
export const PARAM_HELP: Record<string, ParamI18nEntry> = {
  ...
  swa_full: { zh: '滑动窗口注意力层保留全量 KV 而非仅窗口内（质量↑显存↑）。混合注意力模型（线性注意力 + 周期全注意力，模型内置信息卡「全注意力间隔」即此标识）缓存策略由元数据自动决定，通常无需开启；纯 SWA 模型（Gemma/Llama-4 系）长上下文质量下降时的排查手段', en: 'Keep full-size KV for sliding-window attention layers instead of window-only (better quality, more VRAM). Hybrid attention models (linear attention + periodic full attention; see "Full Attn Interval" in model info) manage caching from metadata automatically — usually leave off; a debugging aid for pure-SWA models (Gemma/Llama-4) when long-context quality degrades' },
  ...
  cors_origins: { zh: '允许跨域访问的来源列表；* 表示所有来源', en: 'Comma-separated allowed origins; * allows any origin' },
  cors_methods: { zh: '允许跨域访问的 HTTP 方法列表', en: 'Comma-separated allowed HTTP methods' },
  cors_headers: { zh: '允许跨域携带的请求头；* 表示所有', en: 'Comma-separated allowed headers; * allows any' },
