...
- **Coverage**: 14 mapped parameters (model / alias / ctx_size / temperature / top_k / top_p / min_p / repeat_penalty / presence_penalty / seed / ui / slots_endpoint / metrics / parallel); one real launch on b11178 measured **13 checked + 1 skipped + 0 mismatched** (`ctx_size 262144` equals the engine's `n_ctx 262144`). The other ~50 parameters are simply not reported by the engine, so the UI only speaks up on a real mismatch and never implies everything was verified.
- **`skipWhen` is data, not a branch**: `ctx_size` ↔ `n_ctx` drops out of the comparison when `--fit` is `on`/`auto` — the engine recomputes the context length to fit VRAM, so comparing then always false-reports. Parameters the engine rewrites are declared in the table instead of hard-coded, so adding one never touches the comparison logic.
- **Re-checks are triggered on demand, not by a timer**: `Launcher.recheckProps()` is called through `server:status(refresh:true)`, and the trigger points are "the tab became visible again" (status card `onActivated`) plus the manual "re-verify" button on the preview card, with one automatic check at readiness. No polling: engine parameters can only change via an external `POST /props`, so knocking the port every minute catches nothing and reports nothing, whereas "someone is actually looking" is a signal we can event-ise. **The same-status event is re-emitted only when the serialized result actually changed**, and `PropsCheck.checkedAt` travels with the result so the UI can state how fresh it is.
- **Baseline version drift rides the same channel**: `PropsCheck.baselineDrift` compares the `build_info` from `/props` against `ENGINE_BASELINE_BUILD`, and the preview card warns that the parameter baseline may be stale. This table is a snapshot of one version's help — once the engine is upgraded, the decision basis of those ~50 non-readable parameters stops being trustworthy, and this is the **only** way to notice at runtime (at development time `verify-params-sync` ⑥ keeps the declaration sites consistent).
