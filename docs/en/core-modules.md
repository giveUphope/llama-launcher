...
- **Re-check every 60s**: `POST /props` can mutate global generation properties at runtime, so checking once would let the "verified" conclusion silently go stale. `startPropsWatch` does an initial check plus one every 60s and **re-emits the status event only when the serialized result actually changed** (no change, no cross-bridge noise); the timer is cleared on process exit.
