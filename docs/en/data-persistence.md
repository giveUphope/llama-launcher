...
| `server.ts` | `ServerInfo` (incl. the `values` launch snapshot, the `stop` stop-fact and `envOverrides`, the `LLAMA_ARG_*` variable names detected at launch; `url` is built by `displayHost()` from the possibly comma-separated `host`, picking the loopback or first TCP address, and is empty for a pure `.sock` config), `ServerStatus`, `OutputEntry`, `AppLogEntry`/`AppLogKind` (application log), `ModelInfo` (a model scan entry, incl. `tags`), `LlamaBenchSummary`/`LlamaBenchJobState` (llama-bench offline health check) |
