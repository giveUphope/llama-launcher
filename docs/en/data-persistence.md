...
| `server.ts` | `ServerInfo` (incl. the `values` launch snapshot, the `stop` stop-fact, `envOverrides` — the `LLAMA_ARG_*` variable names detected at launch — and `propsCheck`, the read-back reconciliation whose `checkedAt` timestamp tells the UI how fresh the verdict is (re-checks are triggered on demand, so no interval can be assumed); `url` is built by `displayHost()` from the possibly comma-separated `host`, picking the loopback or first TCP address, and is empty for a pure `.sock` config), `PropsCheck`/`PropsMismatch` (the shape of the `/props` reconciliation result, defined in `params/props-mapping.ts`), `ServerStatus`, `OutputEntry`, `AppLogEntry`/`AppLogKind` (application log), `ModelInfo` (a model scan entry, incl. `tags`), `LlamaBenchSummary`/`LlamaBenchJobState` (llama-bench offline health check) |
