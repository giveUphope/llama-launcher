...
- **参数网格**：`param-grid`（参数设置页）`repeat(auto-fill, minmax(418px, 1fr))`，**不设 `max-width` 上限**（2026-09-20 起，见 STYLE_TODO #76）、gap `4px 14px`、≤720px 单列；装饰一律 Arco 主色蓝 `rgb(var(--primary-6))`。
  ...
  - **不设上限的理由（实测）**：旧 `max-width: 1160px` 硬封顶 3 列，卡片可用宽 ≥1326px 后全部变成右侧空白——1920 空 **486px**、2560 空 **1126px**（近半宽度未用）。现列数随宽度单调增长且 `waste=0`（下表为 #78 紧凑化后逐视口复测；对照 #77 的 450：1156 由 1 列回到 2 列、1600 由 2 列回到 3 列、2560 由 4 列回到 5 列）：

    | 视口 | 网格宽 | 列数 × 轨宽 | 控件宽 | 滑块轨道 | 标签截断 |
    |---|---|---|---|---|---|
    | 1156 | 850 | 2 × 418 | 164 | 80 | 0 |
