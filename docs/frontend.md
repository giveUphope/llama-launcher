...
- **统一控件宽度**：参数控件 `label-col` **定宽 140px**（**必须 `flex: 0 0 140px` + `min-width: 0`**，**右对齐** + `margin-right: 8px`，长标签省略号截断）。取值依据（STYLE_TODO #72/#73 实测）：历史上最长标签「合成接受长度（基准）」自然宽 140px，取设置面板那档 110px（扣 8px 右距后可用 94px）会截断 11/60 行（对齐不能以牺牲可读性为代价；该两条「…（基准）」标签已把限定语并入 tooltip，现全页最长 127px ≤ 可用 132px、零截断）。两条硬判据：① Arco 原生 `flex: 0 0 auto` 让标签按文字宽自适应（实测 28–93px 九种），同列控件起点逐行漂移（x 318→380）；② **不能用设置面板那套 `0 1`（可收缩）**——参数行控件（滑块 + 88px 数字框 + 72px 提示槽）占满行，收缩只会落在标签上（实测缩成 64/83px，控件重新错位）。落点：`ParamRow.vue` 以 `:deep(.arco-form-item-label-col)` 统一施加（每行自带独立 `a-form-item`、无外层 `a-form`，故不走 `label-col-style`）；设置面板三面板仍走组件原生 `label-col-style`（110 / AdvancedPanel 140px）。控件列内：`num-input` 撑满（`IntEntryParam` 的 `a-space` / `a-input-number` `width: 100%`）、下拉触发器 `dropdown-trigger` 宽 100%、滑块项 `flex: 1` + 数字项固定 88px、**GGUF 提示为常驻定宽槽 `.gguf-hint-slot`（`flex: 0 0 72px` + `min-width: 0`）**——提示本身 `v-if` 但槽位恒在，避免行宽随提示出现/消失跳动。**定宽 flex 槽位一律配 `min-width: 0`**（默认 `min-width: auto` 会被长内容撑破：实测 72px 槽被 30 字别名提示撑到 222px，控件反而缩到 146px、输入框内部宽 0px）。
...
- **仍保留原生 `title` 的三类（明确边界，勿顺手改）**：① Arco 组件的 `title` **prop**（`a-modal` / `a-popconfirm` / `a-statistic` / `a-dgroup`）与 `iframe title`（无障碍名）——不是浮层；② **`v-for` 数据条目上的提示**（模型表格行、下载任务行、预设行、`ParamSummaryCard` / `targetRecs` 的 chip）——每条目挂一个 Arco trigger 实例与 §7.1 热路径铁律相冲；③ **截断值提示**（`.mono-val` / `.file-name` / `.task-name` / `.task-error` / `.summary-label` 等长路径与长 URL）与 `a-input :title`（外包 host 会动到表单控件盒型）。实测全站 `:title` 由 **60 → 34 处**（26 处转 `ToolTip`），剩余 34 处逐条核过均属上述三类。
