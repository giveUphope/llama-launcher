...
- **统一控件宽度**：参数控件 `label-col` **定宽 140px**（**必须 `flex: 0 0 140px` + `min-width: 0`**，**右对齐** + `margin-right: 8px`，长标签省略号截断）。取值依据（STYLE_TODO #72/#73 实测）：历史上最长标签「合成接受长度（基准）」自然宽 140px，取设置面板那档 110px（扣 8px 右距后可用 94px）会截断 11/60 行（对齐不能以牺牲可读性为代价；该两条「…（基准）」标签已把限定语并入 tooltip，现全页最长 127px ≤ 可用 132px、零截断）。两条硬判据：① Arco 原生 `flex: 0 0 auto` 让标签按文字宽自适应（实测 28–93px 九种），同列控件起点逐行漂移（x 318→380）；② **不能用设置面板那套 `0 1`（可收缩）**——参数行控件（滑块 + 88px 数字框 + 76px 提示槽 + 24px 还原槽）占满行，收缩只会落在标签上（实测缩成 64/83px，控件重新错位）。落点：`ParamRow.vue` 以 `:deep(.arco-form-item-label-col)` 统一施加（每行自带独立 `a-form-item`、无外层 `a-form`，故不走 `label-col-style`）；设置面板三面板仍走组件原生 `label-col-style`（110 / AdvancedPanel 140px）。控件列内：`num-input` 撑满（`IntEntryParam` 的 `a-space` / `a-input-number` `width: 100%`）、下拉触发器 `dropdown-trigger` 宽 100%、滑块项 `flex: 1` + 数字项固定 88px、**行尾两个装饰一律常驻定宽槽**：`.gguf-hint-slot`（`flex: 0 0 76px`）承载建议值芯片、`.clear-slot`（`flex: 0 0 24px`）承载还原 ✕，**芯片/按钮本身 `v-if`（`v-if` 挂在 `ToolTip` 上以免渲染空 host），槽位恒在**——判据是「同一列 60 行的控件宽与槽位 x 各只有一个值」（#72 曾把 ✕ 判为「瞬时态不预留」，实测残留 `控件宽 [176, 148]` 两值 + 提示槽 x 被左移 28px，用户 2026-09-20 批注后改为预留，见 STYLE_TODO #77）。**唯一不预留的是 `.dep-hint` 警示图标**（4+14px）：`syncDependencies` 会把依赖不满足的参数重置回默认，只有 `file`/`dir` 型保留路径（`resetDep` 早退），而 11 条 `dependsOn` 中 `file` 型仅 `spec_draft_model` 一条 → 该状态最多命中 1/60 行，为它常年预留 18px 会把网格最小轨从 450 推到 468（1728 视口因此少一列），不划算。**定宽 flex 槽位一律配 `min-width: 0`**（默认 `min-width: auto` 会被长内容撑破：实测 72px 槽被 30 字别名提示撑到 222px，控件反而缩到 146px、输入框内部宽 0px）。
...
- **建议值芯片族统一**（`.gguf-hint` 参数行 / `.suggestion-chip` 模型页「建议参数」/ `.rec-chip` 性能目标联动建议，实测全档同值）：`a-tag size="small"`（h20 / 圆角 2px / 内距 `0 8px`）+ `font-family: var(--font-mono)` + `font-size: var(--fs-sm)`；承载 `key = value` 的两族再配 `gap: 5px` + `.chip-key`（`rgb(var(--primary-6))` 600）/`.chip-eq`（`--color-text-3`）/`.chip-val`（`--color-text-1`）三段配色（Arco 不渲染 `.arco-tag-content`，故 gap/align 写在 a-tag 根类上，§7.5.4 ① / #69）。**配色语义唯一**：`color="arcoblue"` 蓝底 = **芯片自身可点击应用**（仅 `.gguf-hint.applicable`）；灰底 = 只展示（应用入口在别处——模型页由卡片按钮批量应用、目标弹层由「应用建议」按钮应用、非建议类的 `.gguf-hint` 只读展示模型自带值）。两处历史离群值已收敛（#77）：`.gguf-hint` 的 `padding-inline: 6px` 覆写（为 72px 旧槽挤出的 2px）与 `.rec-chip` 缺 `size="small"`（实测 h24，与同族 h20 混排）。
- **GGUF 建议值芯片**（`.gguf-hint`，定宽 **76px** 常驻槽 `.gguf-hint-slot` 内）：长值**必须由 JS 做中间省略**（头 5 + `…` + 尾 2，8 字为实测上限：mono 12px ≈ 7.3px/字 + Arco 原生 `0 8px` 内距 → 8 字 74.3px ≤ 76px 槽），不能指望 CSS `text-overflow: ellipsis`——`a-tag` 是 `inline-flex` 且无内容包装层（§7.5.4 ① / #69 同源），CSS 省略号根本不生效，30 字别名建议会被硬切成无提示的 "Qwen3-32B"。尾部保留是因量化后缀/单位才是区分信息；**完整值 + 参数名 + 操作提示三段进 `ToolTip`**（禁原生 `title`，它承载不了多段文本）。
- **参数网格**：`param-grid`（参数设置页）`repeat(auto-fill, minmax(450px, 1fr))`，**不设 `max-width` 上限**（2026-09-20 起，见 STYLE_TODO #76）、gap `4px 14px`、≤720px 单列；装饰一律 Arco 主色蓝 `rgb(var(--primary-6))`。
  - **最小轨 450px 的推导**：一行参数的最小舒适宽 = 边框 2 + 行内距 16 + 标签列 140 + 8 + 控件 ≥176（滑块轨道 80 + 间隙 8 + 数字框 88）+ 4 + 提示槽 76 + 4 + 还原 ✕ 槽 24 = **450**（等价式：`控件宽 = 轨宽 − 274`）。#76 当时写的 400 少算了行的 2px 边框与 16px 内距，实测 413px 轨道上控件只剩 171、轨道 75px，已低于 #73 的 80px 下限；本轮为还原 ✕ 增设常驻槽（+28px）时一并校正。**不得为凑列数下调**：380px 时 1920 虽能排到 4 列，但滑块轨道被压到 **55px**；340px 时 1440 排 3 列、轨道仅 **31px**。
  - **不设上限的理由（实测）**：旧 `max-width: 1160px` 硬封顶 3 列，卡片可用宽 ≥1326px 后全部变成右侧空白——1920 空 **486px**、2560 空 **1126px**（近半宽度未用）。现列数随宽度单调增长且 `waste=0`（下表为 #77 预留两个行尾槽后的复测；对照 #76 旧值：1600 由 3 列退为 2 列、2560 由 5 列退为 4 列，代价即 +28px 预留）：

    | 视口 | 网格宽 | 列数 × 轨宽 | 控件宽 | 滑块轨道 | 标签截断 |
    |---|---|---|---|---|---|
    | 1156 | 850 | 1 × 850 | 566 | 480 | 0 |
    | 1280 | 974 | 2 × 480 | 206 | 110 | 0 |
    | 1440 | 1134 | 2 × 560 | 286 | 190 | 0 |
    | 1600 | 1294 | 2 × 640 | 366 | 270 | 0 |
    | 1728 | 1422 | 3 × 465 | 190.7 | 94.7 | 0 |
    | 1920 | 1614 | 3 × 529 | 254.7 | 158.7 | 0 |
    | 2560 | 2254 | 4 × 553 | 279 | 183 | 0 |

  - 各列宽下对齐不变量恒成立（Playwright 逐视口实测 60 行）：标签列宽 `[140]` 单一值、**控件宽每列恰一个值（不再是 `[176, 148]` 两值）**、提示槽 x 与 ✕ 槽 x 每列各一个值（出现/消失不再平移）、0 行标签截切、0 刻度节点、芯片 0 裁切。
