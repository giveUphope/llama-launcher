...
项目文档集中在 `docs/`，作为项目 Wiki 使用。本文档为首页（功能总览 + 快速开始 + 使用指南），其余按主题分类：
...
- **初次接触项目**：按 [architecture.md](architecture.md) → [core-modules.md](core-modules.md) → [frontend.md](frontend.md) 顺序阅读。
...
- 表格展示模型名称、大小、修改时间，**伴随文件标签**（mmproj / dflash / draft 徽章，按模型目录检测）
...
- **DFlash/推测解码自动检测**：模型切换时自动检测同目录的 dflash/draft 草稿模型并配置对应 `--spec-type`（dflash → `draft-dflash` + `-fa on` + n_max 15）
...
│   └── ui/                          # Vue 3 前端
├── scripts/                         # 构建辅助脚本（打包前清理、符号链接处理等）
└── docs/                            # 分类文档（除 AGENTS.md 外的全部项目文档）
    ├── architecture.md              # 架构与项目结构
    ├── core-modules.md              # 核心模块详解
    ├── params-system.md             # 参数系统
    ├── desktop-main.md              # Electron 主进程
    ├── frontend.md                  # 前端架构 + UI 风格规范
    ...
    ├── workflow.md                  # 开发工作流
    ├── testing.md                   # 测试
    ├── params/                      # 参数对照表 + llama-server help 输出
    ├── style/STYLE_TODO.md          # UI 风格待修复清单
    ├── README.md                    # 本文件（功能总览）
    └── CHANGELOG.md                 # 版本历史
```
...
`shared` 包是所有类型定义、参数表和多语言字典的唯一来源，`core` 和 `ui` 均依赖它。
...
（等效于在 `apps/desktop` 下执行 `pnpm dist`。）输出目录为项目根目录下的 `release/`，生成 Portable 单文件 `llama Launcher 0.0.02.exe`。
...
IPC 常量唯一事实源为 `packages/shared/src/types/ipc.ts`，preload 侧由 `scripts/generate-preload.cjs` 生成，`scripts/verify-ipc-sync.cjs` 在 lint 阶段校验产物未过期。
...
> 全部开发文档的分类导航与「按任务快速定位」见本文档顶部 **📚 项目 Wiki（文档地图）**。本文档其余部分为产品功能总览与使用指南，编写格式遵循下方「文档编写约定」。
...
1. **标题**：`# 文档名`，一句概括主题。
2. **范围说明**：标题下紧跟两行说明块——第一行 `> 范围：<覆盖内容>`，第二行 `> 索引：<README 相对链接> · 相关：<相关文档相对链接>`；说明本文覆盖范围、指向本索引、列出相关文档（docs/ 内一律相对路径）。
...
6. **来源**：拆分自原 CODE_WIKI.md 的章节保持内容原样（仅调整格式），新增内容标注日期；待修复/已知问题登记到 `style/STYLE_TODO.md` 而不是散落在正文。
7. **例外（保持自身格式）**：`CHANGELOG.md`（历史版本记录，按版本分组）、`params/LLAMA_SERVER_PARAMS.md`（由 `scripts/generate-params-doc.cjs` 生成，勿手改）。
