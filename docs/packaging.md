...
**electron-builder 输出文件名注意**：electron-builder 会规范化版本号，剥离 SemVer trailing zeros（如 `0.0.29` → `0.0.5`）。Release tag 保持完整版本号（`v0.0.29`），`.exe` 文件名（`llama Launcher 0.0.5.exe`）与 tag 不严格对应，这是预期行为。
