...
22. **切模型防丢确认 + 基线可视化（2026-08-29，经用户确认）**：切换模型 / 应用 GGUF 建议参数前检测未保存修改，`confirmDiscardDirty` 确认后重建临时基线（防静默丢失）；基线状态由 `BaselineBadge` 双入口展示（参数页顶部 + 服务页状态卡；2026-09 状态卡自服务页迁入概览 `ServiceStatusCard`，徽章随迁），支持就地「恢复基线」与「清除会话」，用户不必进入参数页即可感知"当前参数偏离了哪个基线"。
