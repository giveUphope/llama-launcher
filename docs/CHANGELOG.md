...
- **README 改写为英文**：承接上一条瘦身后的骨架逐段重写（非对译）为英文着陆页——`What it takes off your plate` / `Highlights` / `Quick start` / `How you use it` / `Project layout` / `Tech stack` / `Documentation map` / `License`。三处连带处理：① **徽章锚点随标题改**（`stack.svg` → `#tech-stack`、`license.svg` → `#license`，正文内链 `#documentation-map`；`check-docs-links.cjs` 的 GitHub 风格 slugify 同时支持中英锚点，实测 143 条链接全绿）；② **语言边界写明**，`docs/` 与 `AGENTS.md` 仍是中文，故 Highlights 末注与文档地图表头两处明示"这些文档目前是中文"，避免英文访客点进去有落差；③ **防被好心翻回**：`AGENTS.md` 的 README 条目改为「英文撰写 + 勿改回中文 + 改后跑 `pnpm docs:check`（徽章依赖 `#tech-stack`/`#license`）」，`workflow.md` 文档编写约定引言列出英文段落骨架并声明 docs 为中文。中文原文完整保留在提交 `d818694` 的历史中。验证：`pnpm lint` 全绿（4 包类型检查、IPC 56 通道、文档链接与锚点、i18n 364 键、oxlint 0 告警）。
