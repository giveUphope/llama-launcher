...
- **全站轮询审查：能由事件说清的，就不再定时提问**：用户要求「审查存在轮询机制的内容，优化为不依靠轮询，而是跟随实际变化」。先把源码里**周期性**的计时器逐个数出来（`setInterval(` 在 `packages/*/src` + `apps/desktop/src`，去掉测试与 mock 后 **6 处**），再逐条问「这个值到底什么时候真的会变」。**① `/props` 回读去掉 60s 盲轮询**：会变的只有引擎侧参数，而它只在启动、改参后 `POST /props` 之后才可能变——触发点换成「页面 `onActivated` 一次」+「预览卡刷新按钮」，`IPC.SERVER_STATUS` 多带 `refresh` 参数（默认 `false`，纯读缓存，免得每次状态刷新都敲引擎一下）；同时把对账时刻 `checkedAt` 标到界面上，因为去掉定时复检之后，「已证实」的有效期不再由代码保证。**② 体检（llama-bench）2.5s 轮询 → 主进程推送**：作业完成本来就发生在主进程的 Promise 回调里，推一次就够；新通道 `system:benchOnStatus`（IPC 通道 **56 → 57**），渲染层只在页签激活时 `resyncBench()` 补一次快照，堵住「订阅之前错过迁移」。**mock 侧按同一份契约重做**（`benchJobStates` + 1400ms 模拟推送），否则浏览器预览就成了唯一还在轮询的地方。**③ 外部实例探测：固定 15s → 该探才探 + 空闲退避**：自家服务 running/starting 时不探（此时探到的必是无关进程，每 15s 白敲一次端口）；没有外部实例时按 `15s×2^n` 退避、封顶 60s，而 `host`/`port` 一改、状态一落回 stopped 就立刻探一次。**这是唯一保留的定时探测**——「别人家的进程何时起来」在本机没有任何可订阅的事件源，退避是在发现延迟与无谓请求之间买平衡。**④ 下载进度同值不再跨桥**：120ms 窗口是**测速的时间基**（不算轮询的账），但序列化后与上次相同就不发（`lastEmit` 判据），静默段（写盘卡顿、限速）事件流归零；顺带把 interval 回调体抽成 `speedTick(id)` 方法，去重逻辑才有地方写单测。**保留的另一处**是 1s 运行时长时钟——它显示的就是「此刻第几秒」，改成事件驱动等于换个计时器，且已按 `pageActive` 门控。**验证**：core **401** / ui **79** / e2e:web **15** 全绿，`pnpm lint` 全绿（含 57 通道数与中英双树配对门禁）；浏览器实测体检徽章由推送完成（`pp 868 · tg 168`），全程零轮询；同一条件复查 `setInterval(` 剩 **2** 处（测速窗口 + 时长时钟）。文档同步 `desktop-main`/`ipc-channels`/`frontend`/`testing` 中英双树与 AGENTS.md 的通道数、用例数。

- **上轮「发现但未修」的 6 项全部落地：基线版本双向守 + 回读扩面 + env 归因 + mock 可目测 + 收录 CORS 族**：用户看完清单只说「全做」。**① 基线版本不再只活在注释里**：`ENGINE_BASELINE_BUILD` 成为 shared 常量，运行时用 `/props` 的 `build_info` 与之比对，不一致即由预览卡提示「参数基线可能已过期」（这张表是某个引擎版本 help 的快照，升级后约 50 项不可回读参数的判定基准就不再可信，而此前**没有任何东西**会在运行期发现此事）；开发期新增 `verify-params-sync` ⑥，要求常量与生成器中英两行、README 中英、params-system 中英「当前实测」段共 **6 处**一致，**解析不到同样 fail**（负测试：只把 README 改成 b9999，门禁 exit 1 并点名该文件与该值）。**② 回读扩面**：补 `ctx_size ↔ default_generation_settings.n_ctx`（真机该键存在，上轮漏映射），映射 13 → **14 项**；同时新增数据驱动的 `skipWhen`——`--fit` 为 `on`/`auto` 时引擎按显存重算上下文长度，此时不比，避免假报。**③ env 归因合并**：`envOverrides` 与 `mismatched` 同时存在时改说因果（新键 `cmd_props_mismatch_env`），只有其中之一时各说各话、不甩锅。**④ 复检时机**：当轮做的是「`startPropsWatch` 首查 + 每 60s 复检」（`POST /props` 可运行期改全局生成属性，只查一次会让「已证实」结论陈旧），**那条定时轮询已在下一轮被去掉**（见下一条），改为「页面可见时 + 用户点刷新按钮」按需触发；留下的部分是 **结果序列化后不同才补发事件**，用例覆盖"没变不补发 / 变了补发 / socket 不发请求 / 非 running 不发请求"。**⑤ mock 可目测**：`?demo=props-mismatch` 与 `?demo=props-drift` 让提示行能在浏览器里复现，实现复用 `checkEngineProps`（绝不抄第二套判定——上轮 mock 事故的直接教训）；过程中我自己引入过一处假数据：把运行快照按参数默认值种下，导致普通加载谎报「运行中 ≠ 当前参数」，改为只在演示路径使用预览卡传来的当前值，复验「普通加载 0 行警示」后才收工。**⑥ 收录 CORS 族 + 端点暴露常驻提示**：按 help 原文收录 `--cors-origins`(`*`) / `--cors-methods`(`GET, POST, DELETE, OPTIONS`) / `--cors-headers`(`*`) / `--cors-credentials`(enabled)，归入新分区「安全与跨域」，参数总数 **60 → 64**、server 组 **10 → 14**、分区 **13 → 14**（11 处计数声明由门禁逐条逼着改齐，改前它一次列出全部 10 处漂移）；概览服务状态卡在 API 地址下方常驻 `sec_open_endpoint_hint`，**仅在「未设 API key 且来源为 `*`」时出现**——引擎那行 `security:` 告警滚过日志就没人再看，一次性弹窗也会被点掉。四个默认值连空格都照抄原文，否则一进界面就多发一条 flag。**顺手修门禁自身的缺陷**：`verify-params-sync` 对 `(default: X)` 的解析会截逗号首段，把 `GET, POST, DELETE, OPTIONS` 读成 `get` 再判我们登记错——**门禁把自己的解析错误当成了数据的错**；判据改为「逗号后各段皆为无空格单词 ⇒ 取值列表 ⇒ 条件式强制 note」，并实测确认**不影响 9 条「值, 0 = 含义」解释式条目**（凭空给既有条目加 note 义务只会逼人绕开门禁），同时确认净变动只落在新增的 `cors_methods` 一条上。**⑦ e2e 抓到我引入的真实布局回归**：新英文标签 `Allowed Origins (CORS)` 压进控件 13px，被既有「表单标签几何」判据当场拦下；去掉分区标题已涵盖的 `(CORS)` 后缀后复绿。顺带把该判据里写死的 `60 行` 改为从 `PARAMS.length` 取数（写死的数字每加一个参数就红一次，结局是人去改数字而不是看布局），并**负测试确认判据仍咬得住**——临时把标签改长则英文态用例即红，用 `.bak` 副本还原（不用 `git checkout`）。验证：core **399**（+18）、ui **79**、e2e:web **15**、`pnpm lint` 全绿（含新 ⑥ 号门禁与中英双树配对）；真机 b11178 实例 + 真实 61 键 `session_values` 实弹对账 **14 项映射 / 13 校验 / 1 跳过 / 0 假报**，`drift=null`，`ctx_size 262144 == n_ctx 262144`。文档中英双树同轮镜像（core-modules §4.11 扩到 5 条、params-system §5.1/§5.2、design-decisions 新增第 25 条、frontend 预览卡与状态卡、testing、AGENTS.md 两条约定）。

- **修掉 drift 脚本的折行假阳性 + 落地 `/props` 回读校验（界面从此有「已证实」信号）**：承接上一条的三项发现，本轮把它们变成代码。**① 工具缺陷**：`verify-help-drift.cjs` 按物理行匹配 `(default:`，而长说明会把默认值挤到下一行、取到空串，把一次纯描述改写报成「默认值变化」（本轮 `--host` 即如此，每次 re-pin 都会再踩）。改为先把缩进续行折回条目行、再只认括号形态 `(default: X)`——续行里裸出现的 `default: follows --device` 属说明文字，收进来会取错值并把两个条目错配。提取覆盖 161 → **167** 条；跨版对拍 b11053↔b11178 归零，负测试（把 `--port` 默认改成 9999）仍能报出。**② 回读校验**：新增 `packages/shared/src/params/props-mapping.ts`（`PROPS_FIELD_MAP` 13 项 + 纯函数 `checkEngineProps`，规则放 shared 与发射实现同一处置）与 `packages/core/src/server-props.ts`（`verifyEngineProps`，取数实现 `PropsFetcher` 可注入）；`Launcher` 在检测到 listening 进入 running 后异步 GET `/props`，完成后**补发一次同状态事件**带下 `PropsCheck`（不阻塞状态迁移，渲染层按 status 幂等），`ServerInfo.propsCheck` 同步可见；服务页命令预览卡只在 `mismatched` 非空时出一行橙提示，逐项列 `flag: 发出值 ≠ 回读值`（新增 i18n 键 `cmd_props_mismatch`，zh/en 各 384 键）。**三处归一是实测逼出来的，不做就天天假报**：float32 噪声（发 0.95 回读 `0.949999988079071`，容差 `PROPS_NUM_TOL=1e-4`）、seed 的 uint32 环绕（发 -1 回读 4294967295）、Windows 路径双反斜杠（`normPath`）；`model`/`alias`/`parallel` 标 `onlyWhenSent`——我们没发值时引擎自行派生，比对必假报。**取数失败只标 `error='unreachable'|'bad_payload'`、`mismatched` 留空**：偶发网络失败不能谎报「参数没生效」，那正是本项目一直在消灭的那类问题。**③ 真机自证**：对用户正在跑的 b11178 实例 + settings 里的真实 61 键 `session_values` 做一次实弹对账 → **12 项校验 / 1 项跳过 / 0 项不一致**，`build_info=b11178-f9af9be21`；顺带证实 `GET /props` 在 `endpoint_props=false` 时默认可读，**无需为此开 `--props`**。**④ 一处自纠**：`Launcher` 默认走全局 fetch，会让单测真去请求本机 8080 上用户的实例——改为构造期注入 `propsFetcher`，`launcher.test.ts` 四处全部换成桩。另修自己写的两处测试缺陷（`model` 走 `MODEL_KEY` 特例通道不在 `PARAMS` 里，映射表加 `flag` 兜底；一条守卫用例断言写偏，改成真的拿 `PARAMS` 对拍）。**⑤ 覆盖面诚实化**：`/props` 只回读约 13 项、其余约 47 项引擎不报，故界面只在真的不一致时出声，文档与代码注释都写明「绝不暗示全部核对过」。验证：core **394**（+13：`props-check.test.ts` 10 条真机夹具 + Launcher 3 条接线含「补发同状态事件」「socket 不发请求」）+ ui **79**、`pnpm lint` 全绿。文档：core-modules 新增 **§4.11 引擎回读对账**（补上此前空缺的 4.11 编号）、architecture 结构树两处、ipc-channels `server:status` 载荷、data-persistence `ServerInfo`/`PropsCheck`、frontend 预览卡第三行、params-system §5.2 回读校验一条、design-decisions 第 24 条补记落地、testing 规模与新测试文件行，中英双树同轮镜像。

- **引擎基线重钉至 b11178 + 参数门控全面复查 + 「后端为参数权威」可行性实测结论**：用户要求「更新所有文档；更新 llama 后端基线版本以及参数逻辑，进一步审查是否还有类似的参数门控错误；分析能否直接用后端逻辑、前端只渲染参数设置与变化，从结构上消灭界面渲染与后端实际接受不一致」。**① re-pin（按 params-system §5.5 全流程）**：本机 `llama-b11178-bin-win-vulkan-x64`（`0.5.0-dev / build 11178 / commit f9af9be21`，仓库内 b11053 二进制已不存在）导出 help 后**先审计再替换**——flag **416 ↔ 416，新增 0 移除 0**，应用 69 个 flag 缺失 0；换基线后 `verify-params-sync` 仍报「help 可对拍的 **49 项 engineDefault 全等**」，即**两个版本之间没有任何一个应用参数的引擎默认值发生变化**，`definitions.ts` / `engine-baseline.ts` 取值零改动。同步改版本标注 4 处（`generate-params-doc.cjs` 中英两行、`engine-baseline.ts` 注释、README 中英各一行）并重生成中英对照表。**drift 脚本报的那 1 条「默认值变化」是折行假阳性**（`--host` 的 `(default:` 被挤到下一行），人工比对两侧原文后确认默认仍是 127.0.0.1，并把「看到默认值变化先查是否折行」写进 §5.5。**② 三条真实变化与门控复查**：(a) `--host` 自 b11178 起接受**逗号分隔多地址**（并规定 `::` 只绑 IPv6、地址重叠属未定义行为）——整串当单地址用会拼出 `http://0.0.0.0,::1:8080/` 这种坏 URL，端口探测也会拿一个无法绑定的串去探从而把「已占用」误报成「空闲」；新增 shared 的 `hostList` / `tcpHosts` / `displayHost` 作为唯一解析处：`Launcher.getStatus().url` 取回环/首个 TCP 地址（纯 `.sock` 给空串）、`system:checkPort` 改为对每个 TCP 地址并行探测且任一绑不上即判占用、`system:findFreePort` 要求**全部**地址可绑才算空闲（部分可绑会让启动失败）。(b) help 里 `(env: LLAMA_ARG_*)` 由 139 条增至 **145** 条，新增的 6 条恰好是采样族（TEMPERATURE / TOP_P / MIN_P / REPEAT_PENALTY / PRESENCE_PENALTY / FREQUENCY_PENALTY），60 个应用参数中 **57 个**带该通道——而「值等于引擎缺省就不发射」的成立前提正是「没有别的东西改写缺省值」。实测（畸形 env 值 + 合法命令行参数并用）证明 env 在启动期即被处理、**即便命令行给了合法值也会因 env 解析失败而终止进程**。处置：`Launcher` 启动时检出 `LLAMA_ARG_*` 并随 `ServerInfo.envOverrides` 下发，命令预览卡就地提示「这些改写不会出现在命令里」（新增 i18n 键 `cmd_env_overrides`，zh/en 各 383 键）；启动器**不代为清理或注入** env，只做可见化。(c) `--completion-bash` 实测吐 298 个裸 flag 名（无类型无默认），`--list-devices` 只给设备，`--json-schema` / `-jf` 属约束生成——三者都不能当默认值来源。**复查排除项**：10 个 `dependsOn` 门控参数逐条验证「依赖不满足 ⇒ 不发射」，且 `ParamRow` 已有 `showDepWarning` + 依赖提示（不会显示一个不生效却不说明的值）；下拉选项对拍 help 的初版结论**作废**——我自写的一次性解析器不认多别名行与 `one of:` 续行列表，报出的 6 处「查无条目」与 5 处「STALE」经逐条核对全是探针缺陷（**自己写的探针是首要嫌疑**），改用真门禁的取数口径。**③ 「后端权威」判读**（写入 design-decisions 第 24 条，中英各一条）：**不能整体照做**——llama-server 不提供机器可读的参数事实源（`--verbose` 顶格 `verbosity = 2147483647` 也**不回显解析后的参数集**，显式传的 `--ctx-size 8192 --temp 0.55` 零打印；无 `--dump-config` 类开关；`/capabilities` 实测 404），help 只能给存在性/取值域/默认且仅 49/60 可机械解析（5 条条件式、6 条 help 里没写默认），约 **45/60** 项「不发就等于没发生」；能拿到的运行期真值只有 `/props`（23 键，含 `default_generation_settings.params` 42 字段、`n_ctx`、`total_slots`、`build_info`）与 `/slots` 那十几项。因此结论是「**发射规则唯一化 + 前提可见化**」而不是「界面改成读后端」：规则只有一份（`shared/params/command.ts`，门禁 ⑤ 拦第二份）+ 界面把不可见的前提（env 覆写、运行中≠当前）说清楚。推荐的下一步（本轮结束时待用户定，**已于同日落地，见下一条**）：服务就绪后拉 `/props` 回读校验那十几项，「读回 ≠ 发出」即报警。**④ 验证**：core **381**（+8：`host-env.test.ts` 5 条 + Launcher 3 条覆盖多地址 / socket / env 检出）+ ui **79** 用例全绿、`pnpm lint` 全绿（含引擎缺省对拍与发射实现唯一性）。文档：params-system §5.5 新增 b11178「当前实测」段并把旧的降为「实测参考」（历史数字按原样保留）、design-decisions 新增第 24 条、ipc-channels（`server:status` 载荷与 `system:checkPort` / `findFreePort` 多地址语义）、data-persistence（`ServerInfo` 行）、frontend（预览卡两条提示）、testing（规模与新增测试文件行）、README 中英基线版本、`AGENTS.md`（前提可见化约定 + 单测计数），中英双树同轮镜像。

- **修掉 mock 预览的第二套发射规则：argv 拼装收敛到 `shared/params/command.ts`**：用户报「mock 页面的参数预览也不准确」。根因与 v0.0.47 那次同源、位置不同——浏览器 mock 没有 node 文件系统，拿不到 core 的 `buildCommand`，于是 `demo-mock.ts` 自带一份 `buildDemoPreviewCommand` 简化实现，仍按「值 == 界面初值就不发射」判定，启动器那 4 个基线推荐值在预览里全都看不到（同一份参数实测：旧规则 12 个 token、推荐值 0/4 命中；现规则 20 个 token、4/4 命中，且旧集合是新集合的子集）。修法不是把副本改对，而是**取消副本**：① 纯发射逻辑（`buildArgv` / `argvFromPreviewOptions` / `tokenizeArgs` / `quoteArg` / `formatCommand`，含 `shouldSkip` / `isDependencyMet` / `appendArg`）移到 `packages/shared/src/params/command.ts`（`shared` 本就是参数表与引擎缺省基线的事实源，且对 node 无依赖）；② `core/command-builder.ts` 只剩 `buildCommand`（叠 `exePath` 存在性校验）与 `previewCommand`；③ mock 的 `server.previewCommand` 改调 `formatCommand(buildArgv(argvFromPreviewOptions({ values, settings, includeCustomArgs: false })))`，`includeCustomArgs:false` 与真实侧 `IPC.SERVER_PREVIEW`（`apps/desktop/src/main/ipc/server.ts`）同参——扩展参数只进完整命令。防复发：`verify-params-sync.cjs` 新增第 ⑤ 项，扫 `packages`/`apps`/`e2e`，**除 `shared/params/command.ts` 外任何文件出现 `push(p.flag)` / `push(p.invert_flag)` 即 fail**（负测试：临时放一个含 `cmd.push(p.flag)` 的探针文件，门禁准确报出其路径；删除后还原复绿）。新增 `packages/ui/src/dev/demo-mock.test.ts` 4 条：初值态必须发出 `--load-mode none` / `--fit off` / `-ctk` / `-ctv`，同时断言 `top_k` 初值 40 恰等于引擎缺省因而不发（证明基准是引擎缺省而非「用户动过没有」）、哨兵值 `--chat-template` / `-c` / `-np` 不发射、手改值 `--top-k 20` 照发且 `custom_args` 不进内置命令框、预览与 shared 实现逐字相等（副本再长出来即红）。验证：core **373** + ui **79** 用例（+1 文件 / +4 用例）、`pnpm lint` 全绿、mock 页服务卡实测预览已含 4 个推荐值。文档同步 `docs/{zh,en}/`：core-modules §4.3（新增「文件分工」一条 + 模块表行）、architecture（结构树加 `params/command.ts`、core 行改为「执行侧包装」、脚本清单补唯一性门禁）、frontend（浏览器预览一段改述）、testing（规模 8 文件 / 79 用例 + ui 表新增行）、design-decisions 第 1 条（补「实现只有一份」及其成因）、`AGENTS.md`（`shared` 职责、Command building 拆分约定、`verify-params-sync` 描述、单测计数）。
