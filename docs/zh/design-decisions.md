...
18. **内存参数基线（实测驱动）**：`cache_type_k/v`（q8_0）、`load_mode`（none）、`fit`（off）、`kv_unified`（off，经 `invert_flag` 恒发射 `--no-kv-unified`）作为**基线推荐默认值**内建。**发射语义的两次订正**：2026-09-20 一版曾写「前三项是 dropdown，值等于默认时不下发命令行——后端沿用自身默认（恰好同向）」，这个"同向"是错的：help 里 `-ctk/-ctv` 默认 **f16**、`--load-mode` 默认 **auto**、`--fit` 默认 **on**，与推荐值方向相反，于是三项推荐**从未送达引擎**，用户看到的 q8_0/none/off 只是界面文字（同坑另在 OOM 缓解钮「KV 量化」上表现为点击后命令零变化）。2026-09-25 起发射基准改为 `engineDefault`（见第 1 条与 `shared/params/engine-baseline.ts`），这 3 项**真的会写进命令行**：默认状态发射集合为 `--load-mode none --fit -ctk q8_0 -ctv q8_0` + 9 个 checkbox flag，快照断言固定在 `packages/core/tests/command-builder-definitions.test.ts`。依据 `docs/archive/experiments/plan-kv-split-cli-test.md`（AMD 7900 XTX 24GB + b10429 Vulkan 实测）：f16 KV + mmap + fit on 的长上下文组合会吃满 32GB 系统内存冻结；q8 KV 使 27B@262K 显存需求从 ~35GB 降至 ~25.7GB，`--load-mode none` 加载后释放权重宿主缓冲，`--fit off` 规避显式 ctx/ngl 时 fit 中止的劣化（25.7 vs 36.6 tok/s）。
...
24. **引擎不是参数的权威来源，界面必须自证（2026-09-26 b11178 基线审计）**：提出过的问题能否「直接用后端逻辑、前端只渲染参数设置与变化」以根治界面与实际引擎不一致？实测结论是**不能整体照做**，因为 llama-server 不提供机器可读的参数事实源：help 只能给出「参数是否存在 / 取值域 / 默认值」，且 60 个应用参数里仅 **49 个**的默认值能被机械解析（5 个是条件式如 `(default: 0, 0 = loaded from model)`、6 个 help 里根本没写默认）；`--verbose` 顶格（`verbosity = 2147483647`）也**不回显解析后的参数集**，显式传的 `--ctx-size 8192 --temp 0.55` 一条都不打印；没有 `--dump-config` 一类导出开关（`--completion-bash` 只给 298 个裸 flag 名，`--json-schema` 属约束生成）；HTTP 侧 `/props`·`/slots` 给的是**解析后的生效值**（含 `default_generation_settings.params` 42 字段、`n_ctx`、`total_slots`、`build_info`），恰好可用来校验采样族/上下文/槽位这一小撮，`/capabilities` 在 b11178 实测 404 不存在。因此约 **45/60** 项参数「不发就等于没发生」，界面要自洽只能靠**同一条发射规则只有一份**（见第 1 条末尾）加**把不可见的前提说出来**。本轮据此落三件：① `--host` 多地址解析收进 shared 的 `hostList`/`tcpHosts`/`displayHost`（否则整串当单地址会拼出坏 URL、端口探测误报空闲）；② `ServerInfo.envOverrides` 下发启动时检出的 `LLAMA_ARG_*` 变量名，命令预览卡就地提示「这些改写不会出现在命令里」——b11178 起 57/60 个应用参数带该 env 通道（采样族 6 条为本轮新增），而「值等于引擎缺省就不发射」的成立前提正是没有东西改写缺省值；③ env 畸形值实测会在启动期直接终止进程（即便命令行给了合法值），故该通路只在界面上可见、**启动器不代为清理或注入 env**。④ 同日跟进：`/props` 回读校验已落地（`core/server-props.ts` + `shared/params/props-mapping.ts`，映射 13 项、真机一次启动实测 12 校验 / 1 跳过 / 0 假报），它是界面目前**唯一**「已证实引擎收到了什么」的信号，也正好补上第 ② 条那个前提缺口——env 改写会在回读里现形，而不是只靠提示文字。
