...
- `system:benchLlamaRun` / `system:benchLlamaStatus` / `system:benchOnStatus`：llama-bench 离线体检（状态迁移由主进程推送，status 只在页签激活时补一次）（pp512/tg128，单模型单作业 + 状态轮询，委托 core `llama-bench.ts`）。
