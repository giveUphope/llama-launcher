...
| `server.ts` | `ServerInfo`（含 `values` 启动快照、`stop` 停止事实、`envOverrides` 检出的 `LLAMA_ARG_*` 变量名；`url` 由 `displayHost()` 从可能是逗号分隔多地址的 `host` 中取回环/首个 TCP 地址，纯 `.sock` 配置为空串）、`ServerStatus`、`OutputEntry`、`AppLogEntry`/`AppLogKind`（应用日志）、`ModelInfo`（模型扫描条目，含 `tags`）、`LlamaBenchSummary`/`LlamaBenchJobState`（llama-bench 离线体检） |
