...
export class ProcessRegistry {
  ...
  cleanupWindow(win: BrowserWindow | number): { windowId: number; killedPids: number[]; forced: number[] } {
    ...
    for (const tracked of set) {
      ...
      try {
        tracked.proc.terminate((step, detail) => {
          switch (step) {
            case 'already-dead':
    ...
    // 安全兜底：按可执行文件名扫杀本窗口关联的所有 exe。
    // 即便 PID 树终止因某种原因未命中（如进程以分离方式启动、PID 跟踪丢失），
    // 也能确保窗口所属的 llama-server 进程被清掉，杜绝关闭窗口后的残留"终端/进程"。
    // 作用域限定在已记录的 exe 名（如 llama-server.exe），不会误伤无关程序。
    // 按 exe 名去重扫杀：同一 exe 只 sweep 一次（避免重复 taskkill），
    // 仅在 PID 树终止未必覆盖全部时作为兜底。
    const sweptSet = new Set<string>();
    for (const tracked of set) {
      const name = tracked.exeName;
      if (name && !sweptSet.has(name)) {
        sweptSet.add(name);
        try {
          if (tracked.proc.sweepByName(name)) sweptSet.add(name);
        } catch (e) {
          cleanupLogger.error('cleanup', `sweep by name ${name} failed`, e);
        }
      }
    }
    const swept = [...sweptSet];
    if (swept.length > 0) {
      cleanupLogger.warn('cleanup', `window#${id} swept residual by exe name`, { swept });
    }
